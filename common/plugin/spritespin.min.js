!function($) {
        ...
        for (var id in instances)
            if (instances.hasOwnProperty(id)) {
                ...
                for (i = 0; i < data.mods.length; i += 1)
                    module = data.mods[i],
                    "function" == typeof module[name] && module[name].apply(data.target, [e, data])
            }
    }
    ...
    function padNumber(num, length, pad) {
        for (num = String(num); num.length < length; )
            num = String(pad) + num;
        return num
    }
    ...
    function load(opts) {
        var i, img, src = "string" == typeof opts.source ? [opts.source] : opts.source, count = 0, images = [], targetCount = opts.preloadCount || src.length, completed = !1, firstLoaded = !1, tick = function() {
            count += 1,
            "function" == typeof opts.progress && opts.progress({
            ...
            !completed && count >= targetCount && firstLoaded && "function" == typeof opts.complete && (completed = !0,
    ...
    function detectSubsampling(img, size) {
        ...
        try {
            ...
            return log(err.message, err.stack),
            !1
        }
    }
    function naturalSize(image) {
        if (null != image.naturalWidth)
            return {
    ...
    Spin.namespace = name,
    Spin.mods = {},
    Spin.eventNames = ["mousedown", "mousemove", "mouseup", "mouseenter", "mouseover", "mouseleave", "dblclick", "mousewheel", "touchstart", "touchmove", "touchend", "touchcancel", "selectstart", "gesturestart", "gesturechange", "gestureend"],
    Spin.eventsToPrevent = ["dragstart"],
    Spin.defaults = {
        source: void 0,
        width: void 0,
        height: void 0,
    ...
    Spin.sourceArray = function(path, opts) {
        ...
        opts.frame && (fStart = opts.frame[0],
        fEnd = opts.frame[1]),
        opts.lane && (lStart = opts.lane[0],
        lEnd = opts.lane[1]);
        var i, j, temp, result = [];
    ...
    Spin.measureSource = function(data) {
        ...
        if (1 === data.images.length) {
            if (data.sourceWidth = size.width,
            data.sourceHeight = size.height,
            data.detectSubsampling && detectSubsampling(img, size) && (data.sourceWidth /= 2,
            data.sourceHeight /= 2),
            ...
            detectSubsampling(img, size) && (data.sourceWidth = data.frameWidth = size.width / 2,
    ...
    Spin.updateInput = function(e, data) {
        ...
        void 0 !== e.touches && e.touches.length > 0 ? (data.currentX = e.touches[0].clientX || 0,
        data.currentY = e.touches[0].clientY || 0) : (data.currentX = e.clientX || 0,
        data.currentY = e.clientY || 0),
        void 0 !== data.oldX && void 0 !== data.oldY || (data.oldX = data.currentX,
        data.oldY = data.currentY),
    ...
    Spin.updateFrame = function(data, frame, lane) {
        ...
        data.loop || data.frame !== data.stopFrame || Spin.stopAnimation(data)) : data.wrap ? data.frame = wrap(data.frame, 0, data.frames - 1, data.frames) : data.frame = clamp(data.frame, 0, data.frames - 1),
        void 0 !== lane && (data.lane = lane,
        data.wrapLane ? data.lane = wrap(data.lane, 0, data.lanes - 1, data.lanes) : data.lane = clamp(data.lane, 0, data.lanes - 1)),
        data.lastFrame == data.frame && data.lastLane == data.lane || data.target.trigger("onFrameChanged", data),
    ...
    Spin.stopAnimation = function(data) {
        data.animate = !1,
        data.animation && (window.clearInterval(data.animation),
        data.animation = null)
    }
    ,
    ...
    Spin.setModules = function(data) {
        ...
        for (i = 0; i < data.mods.length; i += 1)
            modName = data.mods[i],
            "string" == typeof modName && (mod = Spin.mods[modName],
            mod ? data.mods[i] = mod : $.error("No module found with name " + modName))
    }
    ,
    ...
    Spin.calculateInnerLayout = function(data) {
        var w = Math.floor(data.width || data.frameWidth || data.target.innerWidth())
          ...
          , mode = data.sizeMode;
        return mode && "scale" != mode ? ("original" == mode ? (css.width = w1,
        css.height = h1) : "fit" == mode ? a1 >= a ? (css.width = w,
        css.height = w / a1) : (css.height = h,
        css.width = h * a1) : "fill" == mode ? a1 >= a ? (css.height = h,
        css.width = h * a1) : (css.width = w,
        css.height = w / a1) : (css.width = w,
        css.height = h),
        css.width = 0 | css.width,
        css.height = 0 | css.height,
    ...
    Spin.setLayout = function(data) {
        ...
        if (data.responsive && "function" == typeof window.getComputedStyle) {
            ...
            if (style.width) {
                var a = w / h;
                w = 0 | Number(style.width.replace("px", "")),
                h = w / a | 0
            }
        }
        data.target.css({
        ...
        data.canvas && (data.canvasRatio = data.canvasRatio || pixelRatio(data.context),
    ...
    Spin.setEvents = function(data) {
        ...
        for (unbind(target),
        j = 0; j < Spin.eventsToPrevent.length; j += 1)
            bind(target, Spin.eventsToPrevent[j], prevent);
        for (i = 0; i < data.mods.length; i += 1) {
            for (mod = data.mods[i],
            j = 0; j < Spin.eventNames.length; j += 1)
    ...
    Spin.create = function(options) {
        ...
        if (data)
            ...
            if (data = $.extend({}, Spin.defaults, options),
            ...
            "canvas" === data.renderer) {
                ...
                canvas.getContext && canvas.getContext("2d") ? (data.canvas = $(canvas),
                data.context = canvas.getContext("2d"),
                $this.append(data.canvas),
                $this.addClass("with-canvas")) : data.renderer = "image"
            }
            data.target = $this,
        ...
        "string" == typeof data.source && (data.source = [data.source]),
        data.mods && (delete data.behavior,
        delete data.module),
        (data.behavior || data.module) && (data.mods = [],
        data.behavior && data.mods.push(data.behavior),
        data.module && data.mods.push(data.module),
    ...
    Spin.registerModule = function(name, module) {
        return Spin.mods[name] && $.error("Module name is already taken: " + name),
        module = module || {},
    ...
    Spin.extendApi = function(methods) {
        ...
        for (key in methods)
            methods.hasOwnProperty(key) && (api[key] ? $.error("API method is already defined: " + key) : api[key] = methods[key]);
        return api
    }
    ,
    $.fn.spritespin = function(obj, value) {
        ...
        if ("api" === obj) {
            ...
            return data.api = data.api || new Spin.Api(data),
            data.api
        }
        if ("destroy" === obj)
        ...
        return "object" == typeof obj ? (obj.target = obj.target || $(this),
        Spin.create(obj),
...
function($) {
    ...
    SpriteSpin.extendApi({
        ...
        updateFrame: function(frame) {
            return SpriteSpin.updateFrame(this.data, frame),
            this
        },
        ...
        playTo: function(frame, options) {
            var data = this.data;
            if (options = options || {},
            options.force || data.frame !== frame) {
                if (options.nearest) {
    ...
    SpriteSpin.extendApi({
        ...
        requestFullscreen: function(opts) {
            ...
            bindChangeEvent(function() {
                isFullscreen() ? (enter(),
                bindOrientationEvent(enter)) : (unbindChangeEvent(),
                unbindOrientationEvent(),
                exit())
            }),
...
function($, SpriteSpin) {
    ...
    function click(e, data) {
        if (!data.loading && data.stage.is(":visible")) {
            ...
            "horizontal" === data.orientation ? (half = target.innerWidth() / 2,
            pos = data.currentX - offset.left) : (half = target.innerHeight() / 2,
...
function($, SpriteSpin) {
    ...
    function dragStart(e, data) {
        data.loading || data.dragging || !data.stage.is(":visible") || (data.dragFrame = data.frame || 0,
        data.dragLane = data.lane || 0,
        data.dragging = !0,
        SpriteSpin.updateInput(e, data))
    }
    function dragEnd(e, data) {
        data.dragging && (data.dragging = !1,
        SpriteSpin.resetInput(data))
    }
    function drag(e, data) {
        if (data.dragging) {
            if (SpriteSpin.updateInput(e, data),
            Math.abs(data.ddX) + Math.abs(data.ddY) > data.scrollThreshold)
                return data.dragging = !1,
                void SpriteSpin.resetInput(data);
            e.preventDefault();
            var angle = 0;
...
function($, SpriteSpin) {
    ...
    function start(e, data) {
        data.loading || data.dragging || !data.stage.is(":visible") || (SpriteSpin.updateInput(e, data),
        data.dragging = !0,
        data.animate = !0,
        SpriteSpin.setAnimation(data))
    }
    ...
    function update(e, data) {
        if (data.dragging) {
            ...
            "horizontal" === data.orientation ? (half = target.innerWidth() / 2,
            delta = (data.currentX - offset().left - half) / half) : (half = data.height / 2,
...
function($, SpriteSpin) {
    ...
    function start(e, data) {
        data.loading || data.dragging || (SpriteSpin.updateInput(e, data),
        data.dragging = !0)
    }
    function update(e, data) {
        if (data.dragging) {
    ...
    function end(e, data) {
        if (data.dragging) {
            data.dragging = !1;
            var dS, dF, frame = data.frame, lane = data.lane, snap = data.swipeSnap, fling = data.swipeFling;
            "horizontal" === data.orientation ? (dS = data.ndX,
            dF = data.ddX) : (dS = data.ndY,
            dF = data.ddY),
            dS > snap || dF > fling ? frame = data.frame - 1 : (dS < -snap || dF < -fling) && (frame = data.frame + 1),
            SpriteSpin.resetInput(data),
            SpriteSpin.updateFrame(data, frame, lane),
            SpriteSpin.stopAnimation(data)
        }
...
function($, SpriteSpin) {
    ...
    function drawSprite(data) {
        ...
        if ("canvas" === data.renderer) {
            ...
            return data.context.clearRect(0, 0, w, h),
            void data.context.drawImage(data.images[0], x, y, data.frameWidth, data.frameHeight, 0, 0, w, h)
        }
        x = -floor(x * data.scaleWidth),
        y = -floor(y * data.scaleHeight),
        "background" === data.renderer ? data.stage.css({
    ...
    SpriteSpin.registerModule("360", {
        onLoad: function(e, data) {
            var w, h;
            if (data.width && data.frameWidth ? data.scaleWidth = data.width / data.frameWidth : data.scaleWidth = 1,
            data.height && data.frameHeight ? data.scaleHeight = data.height / data.frameHeight : data.scaleHeight = 1,
            data.sourceIsSprite = 1 === data.images.length,
            ...
            "canvas" === data.renderer) {
                ...
                data.sourceIsSprite ? (w = floor(data.sourceWidth * data.scaleWidth),
                h = floor(data.sourceHeight * data.scaleHeight)) : (w = floor(data.frameWidth * data.scaleWidth),
                ...
                h = floor(data.sourceHeight * data.scaleHeight)) : w = h = "100%",
...
function($, SpriteSpin) {
    ...
    function onFrame(e, data) {
        ...
        null == scope.timeout && loop(data, scope)
    }
    function scopeFrom(data) {
        data.blurScope = data.blurScope || {};
        var scope = data.blurScope;
        return scope.canvas = scope.canvas || $("<canvas class='blur-layer'></canvas>"),
    ...
    function trackFrame(data, scope) {
        ...
        d >= data.frames / 2 && (d = data.frames - d),
        scope.steps.unshift({
    ...
    function removeOldFrames(frames) {
        ...
        for (i = 0; i < frames.length; i += 1)
            frames[i].alpha <= 0 && toRemove.push(i);
    ...
    function drawFrame(data, scope, step) {
        ...
        if (!(step.alpha <= 0) && img && img.complete !== !1)
            if (context.globalAlpha = step.alpha,
            data.sourceIsSprite) {
    ...
    function tick(data, scope) {
        if (killLoop(data, scope),
        scope.context) {
            var i, step, context = scope.context, d = 0;
            for (context.clearRect(0, 0, data.width, data.height),
            i = 0; i < scope.steps.length; i += 1)
                step = scope.steps[i],
                step.live = Math.max(step.live - step.step, 0),
                step.alpha = Math.max(step.live - .25, 0),
                drawFrame(data, scope, step),
                d += step.alpha + step.d;
            scope.cssBlur && applyCssBlur(scope.canvas, d),
            removeOldFrames(scope.steps),
            scope.steps.length && loop(data, scope)
        }
    }
    var toRemove = [];
...
function($, SpriteSpin) {
    ...
    function init(e, data) {
        ...
        data.easeDamping = max(min(data.easeDamping || .9, .999), 0),
    ...
    function update(e, data) {
        data.dragging && (killLoop(data, data.easeScope),
    ...
    function end(e, data) {
        for (var last, sample, ease = data.easeScope, samples = ease.samples, lanes = 0, frames = 0, time = 0, i = 0; i < samples.length; i += 1)
            ...
            last) {
                ...
                last = sample;
        samples.length = 0,
        time && (ease.ms = data.easeUpdateTime,
        ease.lane = data.lane,
        ease.lanes = 0,
    ...
    function sampleInput(data, ease) {
        for (ease.samples.push({
            ...
            lane: data.dragLane
        }); ease.samples.length > data.easeSamples; )
            ease.samples.shift()
    }
    function killLoop(data, ease) {
        null != ease.timeout && (window.clearTimeout(ease.timeout),
        ease.timeout = null)
    }
    ...
    function tick(data, ease) {
        ...
        SpriteSpin.updateFrame(data, frame, lane),
        data.dragging ? killLoop(data, ease) : Math.abs(ease.frameStep) > .005 || Math.abs(ease.laneStep) > .005 ? loop(data, ease) : killLoop(data, ease)
    }
    var max = Math.max
      , min = Math.min;
...
function($, SpriteSpin) {
    ...
    function draw(e, data) {
        data.galleryFrame === data.frame || data.dragging ? (data.dragging || data.dX != data.gallerydX) && (data.galleryDX = data.DX,
        data.galleryDDX = data.DDX,
        data.galleryStage.stop(!0, !0).animate({
            left: data.galleryOffsets[data.frame] + data.dX
        })) : (data.galleryStage.stop(!0, !1),
...
function($, SpriteSpin) {
    ...
    SpriteSpin.registerModule("panorama", {
        onLoad: function(e, data) {
            ...
            "horizontal" === data.orientation ? (data.scale = data.height / data.sourceHeight,
            data.frames = data.sourceWidth) : (data.scale = data.width / data.sourceWidth,
...
function($, SpriteSpin) {
    ...
    function updateInput(e, data) {
        ...
        var x, y, dx, dy;
        e.touches && e.touches.length ? (x = e.touches[0].clientX || 0,
        y = e.touches[0].clientY || 0) : (x = e.clientX || 0,
        y = e.clientY || 0),
        x /= data.width,
        y /= data.height,
        null == data.zoomPX && (data.zoomPX = x,
        data.zoomPY = y),
        null == data.zoomX && (data.zoomX = x,
        data.zoomY = y),
        dx = x - data.zoomPX,
        dy = y - data.zoomPY,
        data.zoomPX = x,
        data.zoomPY = y,
        e.type.match(/touch/) && (dx = -dx,
        dy = -dy),
    ...
    function onDraw(e, data) {
        ...
        if (!source)
            return void $.error("'zoomSource' option is missing or it contains unsufficient number of frames.");
        var x = data.zoomX
          , y = data.zoomY;
        null != x && null != y || (x = data.zoomX = .5,
        y = data.zoomY = .5),
        x = 100 * x | 0,
        y = 100 * y | 0,
        data.zoomStage.css({
    ...
    function onClick(e, data) {
        ...
        if (!data.zoomClickTime)
            return void (data.zoomClickTime = clickTime);
        var timeDelta = clickTime - data.zoomClickTime
          , doubleClickTime = data.zoomDoubleClickTime || 500;
        return timeDelta > doubleClickTime ? void (data.zoomClickTime = clickTime) : (data.zoomClickTime = 0,
        void ($(this).spritespin("api").toggleZoom() && updateInput(e, data)))
    }
    ...
    function toggleZoom() {
        var data = this.data;
        return data.zoomStage ? data.zoomStage.is(":visible") ? (data.zoomStage.fadeOut(),
        data.stage.fadeIn(),
        !1) : (data.zoomStage.fadeIn(),
        data.stage.fadeOut(),
        !0) : ($.error("zoom module is not initialized or is not available."),
        !1)
    }
